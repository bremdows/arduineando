	...
	*/	
	...
	if (interruptor == 1){
		...
		switch(giroVentilador){
			case 0:
				// GIRO DE 0º - 180º
				for (posicionVentilador = PosicionParcial; posicionVentilador <= 180; posicionVentilador += 1) {
					servoVentilador.write(posicionVentilador); // 
          PosicionParcial = posicionVentilador;
          velocidadVentilador = map( analogRead(potenVelocidadVentilador), 0, 1023, 0, 255);
          analogWrite(ventilador, velocidadVentilador);

          if( digitalRead(pulsadorGiro) == 1){
            servoVentilador.write(PosicionParcial);
            break;
            delay(100);
          }
          if( digitalRead(pulsadorInterruptor) == 1){
            ...
            delay(100);
				...
				// GIRO DE 180º - 0º
				for (posicionVentilador = PosicionParcial; posicionVentilador >= 0; posicionVentilador -= 1) { // giro desde 180 hasta 0 grados
					servoVentilador.write(posicionVentilador);
          PosicionParcial = posicionVentilador;
          velocidadVentilador = map( analogRead(potenVelocidadVentilador), 0, 1023, 0, 255);
          analogWrite(ventilador, velocidadVentilador);
